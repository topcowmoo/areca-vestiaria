...
function ADHDResources() {
  ...
  const [maxRows, setMaxRows] = useState(6.75);
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth <= 640) setMaxRows(8);
      else if (window.innerWidth <= 768) setMaxRows(5.75);
      else if (window.innerWidth <= 820) setMaxRows(5.75);
      else if (window.innerWidth <= 1024) setMaxRows(6.4);
      else if (window.innerWidth <= 1280) setMaxRows(7);
      else setMaxRows(7);
    };
      ...
      <div className="relative z-20 flex flex-col items-center justify-start flex-grow pt-[clamp(50px, 12vh, 110px)] sm:pt-[10vh] pb-12">
        <div className="w-full max-w-[1200px] px-4 py-4 sm:px-6 sm:py-6 mx-auto">
          ...
          >
            ...
            >
              <Directory filteredData={filteredData} maxRows={maxRows} />
            </div>
          </div>
        </div>
      </div>
